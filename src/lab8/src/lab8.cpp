...
void UR3eMoveInterface::drawCircleXY(double radius_meters)
{
  ...
  std::vector<double> target_joint_positions {0, 0, 0, -M_PI_2, -M_PI_2, 0};
  ...
  for (int i = 0; i <= 8; i++) {
    std::vector<double> target_joint_positions {i*M_PI_4, 0, 0, -M_PI_2, -M_PI_2, 0};
...
void UR3eMoveInterface::drawSquareXY(double side_meters)
{
  ...
  /// TODO: Move the arm into a pose to draw the shape by setting a joint-space target
  std::vector<double> target_joint_positions {-1.0*M_PI_4, -(7.0/8.0)*M_PI, -(2.0/8.0)*M_PI, -M_PI_2 + (1.0/8.0)*M_PI, -M_PI_2, 0};
